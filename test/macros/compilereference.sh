...
if [ $1 == "4mu" ]; then
    ...
    echo "Compiling $1 macros"
    g++ -I $ROOTSYS/include -I $ROOFITSYS/include -I ${melaincdir} compilereference_2e2mu_single.C HZZ4LeptonsAnalysis_2e2mu.C `root-config --glibs` `root-config --libs` `root-config --cflags` -L $ROOFITSYS/lib  -lRooFit -lRooFitCore -L ${melalibdir} -lZZMatrixElementMELA   -lZZMatrixElementMEMCalculators -lKaMuCaCalibration -o RunReference2e2mu
