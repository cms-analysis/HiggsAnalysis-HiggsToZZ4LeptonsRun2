#define MonoHiggsAnalysis2e2mu_cxx
#include "MonoHiggsAnalysis2e2mu.h"
#include <TH2.h>
#include <TStyle.h>
//#include <TCanvas.h>
...
void MonoHiggsAnalysis2e2mu::Loop(Char_t *output)
{
   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	cout << "Starting weight + pileup + LineShape= " << newweight << endl;
      ...
      if (isSignal){
        if( ( MC_PDGID->at(3) == 13 && MC_PDGID->at(4) == -13  &&  MC_PDGID->at(5) == 11 && MC_PDGID->at(6) == -11 )
            || ( MC_PDGID->at(3) == 11 && MC_PDGID->at(4) == -11  &&  MC_PDGID->at(5) == 13 && MC_PDGID->at(6) == -13 )){
	...
	for( int i = 3; i  < 7; ++i){
	  ...
	  //if( fabs(MC_PDGID->at(i)) == 13 && ( fabs(MC_ETA->at(i)) >= 2.4 || MC_PT->at(i) <= 5. ) ){
          if( fabs(MC_PDGID->at(i)) == 13 && ( fabs(MC_ETA->at(i)) >= 2.4) ){
            isInAcceptance = 0;
            break;        
          }
          //if( fabs(MC_PDGID[i]) == 11 && ( fabs(MC_ETA[i]) >= 2.5 || MC_PT[i] <= 7. ) ){
          if( fabs(MC_PDGID->at(i)) == 11 && ( fabs(MC_ETA->at(i)) >= 2.5) ){
	...
	iL_loose_e[i]=-999.;
 	...
 	if( RECOELE_PT->at(i) > 7. 
	  ...
	  if( deltaR <= 0.05 ){
	    ...
	    if( debug )cout << "Electron not passing the HLT trigger paths" << endl;
	...
	for(int l = 0; l < N_loose_mu; ++l){ // loop on muons
	  ...
	  cout << "DeltaR= " << deltaR << " " <<  deltaR/pow(RECOPFPHOT_PT->at(iLp[i]),2) << endl;
	...
	for(int e = 0; e < N_loose_e; ++e){
	  ...
	  if( deltaR<=0.3 && (RECOELE_scl_Eta->at(iL_loose_e[e])< 1.479 || deltaR>0.08) ){ // 0.3 in 76x              
	...
	int tag;
	  ...
	  if (fabs(RECOMU_SIP->at(iL[i]))>=4.) continue; // SIP cut
	  if (fabs(RECOMU_SIP->at(iL[j]))>=4.) continue;
	  if (fabs(RECOMU_PFX_dB_new.at(iL[i]))>=0.35) continue; // Isolation
	  if (fabs(RECOMU_PFX_dB_new.at(iL[j]))>=0.35) continue;
	  
	  if(RECOMU_CHARGE->at( iL[j] ) == RECOMU_CHARGE->at( iL[i] )) continue; // opposite charge

	  cout << "\n Pairing muons with pT= " << RECOMU_PT->at( iL[i] ) << " and " <<  RECOMU_PT->at( iL[j] ) << endl;
	  ...
	  if (fabs(RECOELE_SIP->at(iLe[i]))>=4.) continue; // SIP cut
	  if (fabs(RECOELE_SIP->at(iLe[j]))>=4.) continue;
	  if (fabs(RECOELE_PFX_rho_new.at(iLe[i]))>=0.35) continue; // Isolation cut
	  if (fabs(RECOELE_PFX_rho_new.at(iLe[j]))>=0.35) continue;
	  ...
	  if( has_FSR_Z ){ // if Z has FSR
	    ...
	    if( debug ) cout  << "Z Isolation: "  
			      << "\n RECOELE_PFX_rho_new->at( iLe[i] ) " << RECOELE_PFX_rho_new.at( iLe[i] )
			      << "\n RECOELE_PFX_rho_new->at( iLe[j] ) " << RECOELE_PFX_rho_new.at( iLe[j] )
			      << endl;
	...
	Lepton1qcd.SetPtEtaPhiM(RECOMU_PT->at(ileptonsmumu.at(0)), RECOMU_ETA->at(ileptonsmumu.at(0)), RECOMU_PHI->at(ileptonsmumu.at(0)), 0.105);
	Lepton2qcd.SetPtEtaPhiM(RECOMU_PT->at(ileptonsmumu.at(1)), RECOMU_ETA->at(ileptonsmumu.at(1)), RECOMU_PHI->at(ileptonsmumu.at(1)), 0.105);
	...
	// ee	
	cout << "Checking ee pairs for QCD rejection" << endl;       
	Lepton1qcd.SetPtEtaPhiM(RECOELE_PT->at(ileptonsee.at(0)), RECOELE_ETA->at(ileptonsee.at(0)), RECOELE_PHI->at(ileptonsee.at(0)), 0.000511);
	Lepton2qcd.SetPtEtaPhiM(RECOELE_PT->at(ileptonsee.at(1)), RECOELE_ETA->at(ileptonsee.at(1)), RECOELE_PHI->at(ileptonsee.at(1)), 0.000511);
	DiLeptonQCD=Lepton1qcd+Lepton2qcd;       
	mass = DiLeptonQCD.M();
	...
	for (int i=0;i<2;i++){ 
	  for (int j=0;j<2;j++){
	    ...
	    if ( RECOMU_CHARGE->at(ileptonsmumu.at(i)) == RECOELE_CHARGE->at(ileptonsee.at(j)) ) continue; // shoud be OS	  
	    // evaluate the mass
	    double mass;
...
cout << "more than one Z2 (couple to \the same Z1), choose the one with the highest-pT Z2 leptons" << endl;    
       int indexZ2tmp=-999;
     ...
     if (Z1tag==1) cout << "PTs= " << RECOMU_PT->at(indexlep1Z1) << " " << RECOMU_PT->at(indexlep2Z1) << " " <<  RECOELE_PT->at(indexlep1Z2) << " " << RECOELE_PT->at(indexlep2Z2)<< endl; 

     if (Z1tag==0) cout << "PTs= " << RECOELE_PT->at(indexlep1Z1) << " " << RECOELE_PT->at(indexlep2Z1) << " " <<  RECOMU_PT->at(indexlep1Z2) << " " << RECOMU_PT->at(indexlep2Z2)<< endl; 
	...
	N_4c_w=N_4c_w+newweight;
      ...
      double tmp_pt[4];
      if (Z1tag==1) cout << "PTs= " << RECOMU_PT->at(indexlep1Z1) << " " << RECOMU_PT->at(indexlep2Z1) << " " <<  RECOELE_PT->at(indexlep1Z2) << " " << RECOELE_PT->at(indexlep2Z2)<< endl;
      if (Z1tag==0) cout << "PTs= " << RECOELE_PT->at(indexlep1Z1) << " " << RECOELE_PT->at(indexlep2Z1) << " " <<  RECOELE_PT->at(indexlep1Z2) << " " << RECOELE_PT->at(indexlep2Z2)<< endl;
      ...
      for(int i = 0; i < 2; ++i){ 
	if (Z1tag==1) tmp_pt[i] =  RECOMU_PT->at(indexleptonfinal[i]);
	if (Z1tag==0) tmp_pt[i] =  RECOELE_PT->at(indexleptonfinal[i]);
	cout << tmp_pt[i] << endl;
      }
      for(int i = 2; i < 4; ++i){ 
	if (Z2tag==1) tmp_pt[i] =  RECOMU_PT->at(indexleptonfinal[i]);
	if (Z2tag==0) tmp_pt[i] =  RECOELE_PT->at(indexleptonfinal[i]);
	cout << tmp_pt[i] << endl;
      }
	...
	for (int j=0;j<RECO_NMU;j++) {	    
	  if (RECOMU_PT->at(j)==sortedpT[i]) {
	    ismuon=true;
	    break;
	  }
	  else {
	    for (int j=0;j<RECO_NELE;j++) {	    
	      if (RECOELE_PT->at(j)==sortedpT[i]) {
		iselectron=true;
		break;
	      }
	    }
	...
	if (ismuon){
	  for( int p = 0; p < Nphotons; ++p ){
	    if( iLp_l[ p ] == ipt[i] && iLp_tagEM[ p ] == 0 )  {
	      cout << "Muon with pT= " << RECOMU_PT->at(ipt[i]) << " has associated a photon with pT= " << RECOPFPHOT_PT->at(iLp[p]) <<  endl;
	      flagFSR_tag=0;
	      pfsr=p;
	      break;
	    }
...
void MonoHiggsAnalysis2e2mu::printelebnn(int i){
  bnn_file
    << RECOELE_PT->at(i) << " "
    << RECOELE_ETA->at(i) << " "
    << RECOELE_PHI->at(i) << " "
    << RECOELE_CHARGE->at(i) << " "
    << RECOELE_PFX_rho->at(i) << " "    
    << RECOELE_SIP->at(i) << " ";
}

void MonoHiggsAnalysis2e2mu::printmubnn(int i){
  bnn_file
    << RECOMU_PT->at(i) << " "
    << RECOMU_ETA->at(i) << " "
    << RECOMU_PHI->at(i) << " "
...
double MonoHiggsAnalysis2e2mu::EAele(int index,bool use2011EA){
...
double MonoHiggsAnalysis2e2mu::DELTAPHI( double phi1, double phi2 ){
...
double MonoHiggsAnalysis2e2mu::masserror( std::vector<TLorentzVector> Lep, std::vector<double> pterr){ 
