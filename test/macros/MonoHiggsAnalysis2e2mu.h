...
#ifndef MonoHiggsAnalysis2e2mu_h
#define MonoHiggsAnalysis2e2mu_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   MonoHiggsAnalysis2e2mu(TTree *tree=0,Double_t weight_=1.,string DATA_type_="DATA",string MC_type_="MC");
   virtual ~MonoHiggsAnalysis2e2mu();
   Double_t weight;
...
#ifdef MonoHiggsAnalysis2e2mu_cxx
MonoHiggsAnalysis2e2mu::MonoHiggsAnalysis2e2mu(TTree *tree,Double_t weight_, string DATA_type_, string MC_type_) : fChain(0) 
{
...
MonoHiggsAnalysis2e2mu::~MonoHiggsAnalysis2e2mu()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t MonoHiggsAnalysis2e2mu::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MonoHiggsAnalysis2e2mu::LoadTree(Long64_t entry)
{
...
void MonoHiggsAnalysis2e2mu::Init(TTree *tree)
{
...
Bool_t MonoHiggsAnalysis2e2mu::Notify()
{
...
void MonoHiggsAnalysis2e2mu::Show(Long64_t entry)
{
...
Int_t MonoHiggsAnalysis2e2mu::Cut(Long64_t entry)
{
