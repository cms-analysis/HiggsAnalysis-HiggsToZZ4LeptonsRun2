...
while [ $n -lt ${nlines} ]; do
    ...
    if [ $1 = ${SCERN} ]; then
     ...
     cat submit_HZZ4LeptonsAnalysis_FNAL.sh | sed "s?site?$1?g" | sed "s?mc?$3?g" |sed "s?year?$2?g" | sed "s?HZZ4LeptonsAnalysis?RunReferenceAnalysis?g" | sed "s?jobdir?jobs$4_25ns?g" | sed "s?histodir?histos$4_25ns?g" | sed "s?output?output_${samplename}?g" | sed "s?RunReferenceAnalysis?RunReference$4_signal?g" | sed "s?sig_input_h150.txt?SigCards$4$3/sig_input_${n}.txt?g" | sed "s?_log?_${samplename}_$4.log?g" > jobs$4/submit_ReferenceAnalysis_sig_${samplename}_$4.sh
     cat condor_template.cfg  | sed "s?submit_HZZ4LeptonsAnalysis_FNAL?submit_ReferenceAnalysis_sig_${samplename}_$4?g" | sed "s?RunReferenceAnalysis?RunReference$4_signal?g" | sed "s?sig_input_h150.txt?SigCards$4$3/sig_input_${n}.txt?g" | sed "s?mail?`whoami`?g" > jobs$4/condor_ReferenceAnalysis_sig_${samplename}_$4.cfg
    elif  [ $1 = ${SDESY} ]; then
