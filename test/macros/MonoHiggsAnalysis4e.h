/**   Mono Higgs analysis fram work
 *
 *  Analysis Macro for H->ZZ-> 4 electrons + MET analysis
 *
 ...
 *  CMSSW_7_4_5     
 */
#ifndef MonoHiggsAnalysis4e_h
#define MonoHiggsAnalysis4e_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   vector<float>   *RECOMU_PT_MuHLTMatchPAT;
   vector<float>   *RECOMU_PT_MuHLTMatch_asym_PAT;
   vector<float>   *RECOELE_PT_EleHLTMatchPAT;
   ...
   MonoHiggsAnalysis4e(TTree *tree=0,Double_t weight_=1.,string DATA_type_="DATA",string MC_type_="MC");
   virtual ~MonoHiggsAnalysis4e();
   Double_t weight;
   ...
   double EAele(int indexbis,bool use2011EA);
...
#endif
#ifdef MonoHiggsAnalysis4e_cxx
MonoHiggsAnalysis4e::MonoHiggsAnalysis4e(TTree *tree,Double_t weight_,string DATA_type_,string MC_type_) : fChain(0) 
{
  weight = weight_;
  ...
  if (tree == 0) {
    TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("roottree_leptons_vector.root");
    if (!f || !f->IsOpen()) {
      f = new TFile("roottree_leptons_vector.root");
    }
...
MonoHiggsAnalysis4e::~MonoHiggsAnalysis4e()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t MonoHiggsAnalysis4e::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MonoHiggsAnalysis4e::LoadTree(Long64_t entry)
{
...
void MonoHiggsAnalysis4e::Init(TTree *tree)
{
   ...
   RECOMU_PT_MuHLTMatchPAT = 0;
   RECOMU_PT_MuHLTMatch_asym_PAT = 0;
   RECOELE_PT_EleHLTMatchPAT = 0;
   MC_LEPT_PT = 0;
   MC_LEPT_ETA = 0;
   MC_LEPT_PHI = 0;
   MC_LEPT_THETA = 0;
...
Bool_t MonoHiggsAnalysis4e::Notify()
{
...
void MonoHiggsAnalysis4e::Show(Long64_t entry)
{
...
Int_t MonoHiggsAnalysis4e::Cut(Long64_t entry)
{
...
/*     << RECOELE_SIP->at(i) << " " */
/*     //              << RECOELE_gsftrack_dxy[i] << " " */
/*     //              << RECOELE_gsftrack_dz[i] << " " */
/*     //              << RECOELE_scl_Eta[i] << " " */
/*     << RECOELE_mvaNonTrigV0->at(i) << " "; */
/* } */
#endif // #ifdef MonoHiggsAnalysis4e_cxx
