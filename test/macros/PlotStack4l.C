...
PlotStack4l::PlotStack4l(){
  ...
  //std::string histolabel = "hMZ1BB_3";    // Z1 mass 
  //std::string histolabel = "hMZ1EE_3";    // Z1 mass 
  //std::string histolabel = "hPtZ1_3"; // Z1 pt 
  //std::string histolabel = "hYZ1_3"; // Z1 Y

  //std::string histolabel = "hIsoLep1_3";    // isolation of lowest pT lepton from Z1 
  //std::string histolabel = "hSipLep2_3";  // sip the lowest pT lepton from Z1
  //std::string histolabel = "hIpLep2_3";   // IP of the lowest pT lepton from Z1
  //std::string histolabel = "hIpErLep2_3";   // Iperror of the lowest pT lepton from Z1
...
//   ll->AddText(0.65, 0.6, text);
  ...
  TLegend* legend = new TLegend( 0.55, 0.60, 0.9, 0.92);
...
//   Color_t h400Color = kRed;
  ...
  if (histlabel.find("hMZ1_3")<10 && whichchannel.find("4#mu")<20){
    hframe= new TH2F("hframe","hframe",80,60.,160.,500,0.4,1700000000.);// mZ1 mumu
    hframe2= new TH2F("hframe2","hframe2",6000, 60., 160., 1000, 0.5, 2.);// mZ1 mumu
  }
  ...
  if (histlabel.find("hMZ1_3")<10 && whichchannel.find("4e")<20){
  ...
  if (histlabel.find("hPFMET_3")<10){
    hframe= new TH2F("hframe","hframe",1000, 0., 1000., 1000, 0.0004, 10000000.);// PFMET
  ...
  if ((histlabel.find("hMZ1_3")<10 || histlabel.find("hMZ1_8")<10) && whichchannel.find("4#mu")<20) 
    hframe->SetXTitle("M_{Z#rightarrow#mu#mu} [GeV]");
  if ((histlabel.find("hMZ1_3")<10 || histlabel.find("hMZ1_8")<10) && whichchannel.find("4e")<20) 
  ...
  if (histlabel.find("hMjj_3")<10) hframe->SetXTitle("di jet mass"); //Mjj
  if (histlabel.find("hDjj_3")<10) hframe->SetXTitle("#Delta#eta jets"); //deltaetajj
  ...
  for ( int datasetId=Vdatasetnamebkg.size()-1; datasetId >=0; datasetId--){  
	...
	hfourlepbestmass_4l_afterSel_new_VH->SetLineColor(kOrange-1); 
        ...
        char temp[328];
        sprintf(temp,"%s",histosdir.c_str());
        if(datasetnamebkg.find("WminusH") < 200 && datasetnamebkg.find(temp) < 200 && (datasetnamebkg.find(whichenergy) < 200 || datasetnamebkg.find(whichsample) < 200) && hfourlepbestmass_4l_afterSel_new_WH->GetEntries()>0. ) legend->AddEntry(hfourlepbestmass_4l_afterSel_new_WH,"WH, m_{H}=125 GeV/c^{2}", "F");  
        //hfourlepbestmass_4l_afterSel_new_new->Draw("sameP");    
      }
	...
	// TTT 
	if(datasetnamebkg.find("TT_T") < 200){ 
	...
	if(datasetnamebkg.find(temp) < 200 && datasetnamebkg.find("s-channel") < 200 ){ // provided that this is the last single-top sample
    ...
    if(datasetnamebkg.find(tempp) < 500) {
	  ...
	  (datasetnamebkg.find("output_GluGluHToZZTo4L") < 400 && datasetnamebkg.find(whichenergy.c_str())<200)
					       )
	  )  { 
	 ...
	 htotalHisto->Add(hfourlepbestmass_4l_afterSel_new_ggH); 
       ...
       if(datasetnamebkg.find(temppp) < 400 && (
          ...
          (datasetnamebkg.find("output_WminusH") < 400 && datasetnamebkg.find(whichenergy.c_str())<200)
                                               )
          )  { 
         cout << "Adding WH" << endl;
	  ...
	  (datasetnamebkg.find("output_ZH") < 400 && datasetnamebkg.find(whichenergy.c_str())<200)
					       )
	  )  { 
	 cout << "Adding ZH" << endl;
	 ...
	 htotalHisto->Add(hfourlepbestmass_4l_afterSel_new_ZH); 
	  ...
	  (datasetnamebkg.find("output_ttH") < 200 && datasetnamebkg.find(whichenergy.c_str())<200)
					       )
	  )  { 
	 ...
	 htotalHisto->Add(hfourlepbestmass_4l_afterSel_new_qcdBC);
       ...
       if(datasetnamebkg.find(temppp) < 200 && datasetnamebkg.find("output_ST_") < 200 && datasetnamebkg.find("s-channel") < 200 ){     	 
  ...
  for ( int datasetIdSig=Vdatasetnamesig.size()-1; datasetIdSig >=0; datasetIdSig--){  
	...
	hfourlepbestmass_4l_afterSel_new_signalgg125->SetBinContent(nbins,double(bincontgg125[nbins-1]));
      ...
      if (datasetnamesig.find(temp)< 100 && (datasetnamesig.find("output_VBF_HToZZTo4L_M-125") < 180 || datasetnamesig.find("output_VBF_ToHToZZTo4L_M-125") < 180) ) {
  ...
  */
  ...
  if (histlabel.find("hM4l_7")<10 || histlabel.find("hM4l_8")<10 || histlabel.find("hMELA_8")<10 || histlabel.find("hPFMET_8")<10){
    ...
    legend->AddEntry(hfourlepbestmass_4l_afterSel_new_monoH_MZP600,"m_{Z'}=600 GeV/c^{2}, m_{A_{0}}=300 GeV/c^{2}","L");
    legend->AddEntry(hfourlepbestmass_4l_afterSel_new_monoH_MZP800,"m_{Z'}=800 GeV/c^{2}, m_{A_{0}}=300 GeV/c^{2}","L");
    legend->AddEntry(hfourlepbestmass_4l_afterSel_new_monoH_MZP1000,"m_{Z'}=1 TeV/c^{2}, m_{A_{0}}=300 GeV/c^{2}","L");
...
void PlotStack4l::setSamplesNames4l()
{
  ...
  while(std::getline(infile,inputfilename)){
    ...
    // background higgs
      ...
      if( (inputfilename.find("M125")<200 && (inputfilename.find("GluGluHToZZTo4L_M125_13TeV_powheg_JHUgen_pythia8")<200)) ){
	Vdatasetnamebkg.push_back(inputfilename);
	...
	//cout << "VBF H" << endl;
      ...
      if( (inputfilename.find("M125")<200 && (inputfilename.find("ZH")<200)) ){
	Vdatasetnamebkg.push_back(inputfilename);
...
//       } 
	...
	Vcolorbkg.push_back(kTeal-6);
      ...
      if(inputfilename.find("TTTo2L2Nu2B")<200){
	Vdatasetnamebkg.push_back(inputfilename);
