...
//   TFile *mu_scale_factors = new TFile("SF2013_V5_old2011.root");
   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      //if (!(Run==1 && Event==357635 && LumiSection==3577)) continue;
	...
	*/
	...
	int tag;
	  ...
	  cout << "\n Looking for a new muon pair"<< endl;
	  ...
	  cout << "\n Looking for a new electron pair"<< endl;
      ...
      // Isolation cut on electorn and muons  
	...
	for (int index2=index1+1; index2<Zcandvector.size();index2++){
	  if (Zcandvector.at(index1).tag==Zcandvector.at(index2).tag) continue;
	  ...
	  if (Zcandvector.at(index1).withFSR==1 && Zcandvector.at(index2).withFSR==0 ) {  // correct the muon isol. of all the 4 leptons for the photon pT asosciated to first Z
	    ...
	    if (Zcandvector.at(index1).tag==1){
	    ...
	    if (Zcandvector.at(index1).tag==0){
	      if(Zcandvector.at(index1).ilept1_FSR==true){
	    ...
	    if (Zcandvector.at(index1).tag==1 && 
		(Zcandvector.at(index1).isol1 >=0.35 || Zcandvector.at(index1).isol2 >=0.35)) continue;
	    if (Zcandvector.at(index1).tag==0 && 
		(Zcandvector.at(index1).isol1 >=0.5 || Zcandvector.at(index1).isol2 >=0.5)) continue;
	  
	    if (Zcandvector.at(index2).tag==1 && 
		(Zcandvector.at(index2).isol1 >=0.35 || Zcandvector.at(index2).isol2 >=0.35)) continue;
	    if (Zcandvector.at(index2).tag==0 && 
		(Zcandvector.at(index2).isol1 >=0.5 || Zcandvector.at(index2).isol2 >=0.5)) continue;
	    ...
	    else if (Zcandvector.at(index2).tag==0){ 
	      if(Zcandvector.at(index2).ilept1_FSR==true){
	    ...
	    else if (Zcandvector.at(index1).tag==0){
	      if(Zcandvector.at(index1).ilept1_FSR==true){
		EffectiveArea=EAele(Zcandvector.at(index1).ilept1,tag_2011);
		Zcandvector.at(index1).isol1=(RECOELE_PFchHad[Zcandvector.at(index1).ilept1]+
					      max(0.,RECOELE_PFneuHad[Zcandvector.at(index1).ilept1]+
		...
		(Zcandvector.at(index1).isol1 >=0.35 || Zcandvector.at(index1).isol2 >=0.35)) continue;
	    if (Zcandvector.at(index1).tag==0 && 
		(Zcandvector.at(index1).isol1 >=0.5 || Zcandvector.at(index2).isol2 >=0.5)) continue;
