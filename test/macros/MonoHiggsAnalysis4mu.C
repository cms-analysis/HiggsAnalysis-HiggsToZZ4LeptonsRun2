...
if (mass > max_mass) max_mass = mass;
 ...
 // Loose lepton identification
    ...
    //commented by sherif
    for (int i=0;i<2;i++){
      RECOMU_PFX_dB_new[i]=RECOMU_PFX_dB->at(i);
      }
    ...
    for( int i = 0; i < RECO_NELE; ++i ){
      ...
      //GIORGIA: BDT DIFFERENT FROM NICOLA, TO BE CHECKED
      bool BDT_ok = 0; // Fixed Phys14-trained MVA  
	...
	if( RECOPFPHOT_PT->at(iLp[i]) > 2. ){
	  ...
	  RECOPFPHOT_DR.at(iLp[i])=min_deltaR; 
	}
      }
    ...
    for(int i=0.;i<Nphotons;i++) {
      if (iLp_l[i]!=-1 && iLp_tagEM[i]==0) cout << "There is photon with pT= " << RECOPFPHOT_PT->at(i) << " attached to a muon with pT= " << RECOMU_PT->at(iLp_l[i]) << endl;
    };
    // *** end FSR
	...
	if (fabs(RECOMU_SIP->at(iL[i]))>=4.) continue;
	...
	EZ = RECOMU_E->at( iL[i] ) + RECOMU_E->at( iL[j] );
	cout << "\n Looking for a new pair"<< endl;
	if (debug) cout << "pt,eta,phi,charge= " << RECOMU_PT->at( iL[i] ) << " " << RECOMU_ETA->at( iL[i] ) << " " << RECOMU_PHI->at( iL[i] ) << " " << RECOMU_CHARGE->at( iL[i] )<< endl;
	if (debug) cout << "pt,eta,phi,charge= " << RECOMU_PT->at( iL[j] ) << " " << RECOMU_ETA->at( iL[j] ) << " " << RECOMU_PHI->at( iL[j] ) << " " << RECOMU_CHARGE->at( iL[j] )<< endl;

	pxZ = RECOMU_PT->at( iL[i] )*cos( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*cos( RECOMU_PHI->at( iL[j] ) );
	pyZ = RECOMU_PT->at( iL[i] )*sin( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*sin( RECOMU_PHI->at( iL[j] ) );
	pzZ = RECOMU_P->at( iL[i] )*cos( RECOMU_THETA->at( iL[i] ) ) + RECOMU_P->at( iL[j] )*cos( RECOMU_THETA->at( iL[j] ) );
	...
	sum_ptZ = RECOMU_PT->at( iL[i] ) + RECOMU_PT->at( iL[j] );
	...
	for( int p = 0; p < Nphotons; ++p ){
	  ...
	  // evaluate the mass
	    ...
	    E = RECOMU_E->at( iL[i] ) + RECOMU_E->at( iL[j] ) + fabs( RECOPFPHOT_PT->at(iLp[p])/sin( RECOPFPHOT_THETA->at(iLp[p]) ) );
	    px = RECOMU_PT->at( iL[i] )*cos( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*cos( RECOMU_PHI->at( iL[j] ) ) + RECOPFPHOT_PT->at(iLp[p])*cos( RECOPFPHOT_PHI->at(iLp[p]) );
	    py = RECOMU_PT->at( iL[i] )*sin( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*sin( RECOMU_PHI->at( iL[j] ) ) + RECOPFPHOT_PT->at(iLp[p])*sin( RECOPFPHOT_PHI->at(iLp[p]) );
	    pz = RECOMU_P->at( iL[i] )*cos( RECOMU_THETA->at( iL[i] ) ) + RECOMU_P->at( iL[j] )*cos( RECOMU_THETA->at( iL[j] ) )
	...
	if( has_FSR_Z ){ // if Z has FSR
	  ...
	  if( N_FSR_Z == 1 ){
	    ...
	    if( pi > -1 ){
	      ...
	      E = RECOMU_E->at( iL[i] ) + RECOMU_E->at( iL[j] ) + fabs(RECOPFPHOT_PT->at(iLp[pi])/sin( RECOPFPHOT_THETA->at(iLp[pi]) ));
	      px = RECOMU_PT->at( iL[i] )*cos( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*cos( RECOMU_PHI->at( iL[j] ) ) + RECOPFPHOT_PT->at(iLp[pi])*cos( RECOPFPHOT_PHI->at(iLp[pi]) );
	      py = RECOMU_PT->at( iL[i] )*sin( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*sin( RECOMU_PHI->at( iL[j] ) ) + RECOPFPHOT_PT->at(iLp[pi])*sin( RECOPFPHOT_PHI->at(iLp[pi]) );
	      pz = RECOMU_P->at( iL[i] )*cos( RECOMU_THETA->at( iL[i] ) ) + RECOMU_P->at( iL[j] )*cos( RECOMU_THETA->at( iL[j] ) )
		+ fabs(RECOPFPHOT_PT->at(iLp[pi])/sin( RECOPFPHOT_THETA->at(iLp[pi]) ))*cos( RECOPFPHOT_THETA->at(iLp[pi]) );
	      ...
	      E = RECOMU_E->at( iL[i] ) + RECOMU_E->at( iL[j] ) + fabs(RECOPFPHOT_PT->at(iLp[pj])/sin( RECOPFPHOT_THETA->at(iLp[pj]) ));
	      px = RECOMU_PT->at( iL[i] )*cos( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*cos( RECOMU_PHI->at( iL[j] ) ) + RECOPFPHOT_PT->at(iLp[pj])*cos( RECOPFPHOT_PHI->at(iLp[pj]) );
	      py = RECOMU_PT->at( iL[i] )*sin( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*sin( RECOMU_PHI->at( iL[j] ) ) + RECOPFPHOT_PT->at(iLp[pj])*sin( RECOPFPHOT_PHI->at(iLp[pj]) );
	      pz = RECOMU_P->at( iL[i] )*cos( RECOMU_THETA->at( iL[i] ) ) + RECOMU_P->at( iL[j] )*cos( RECOMU_THETA->at( iL[j] ) )
		  ...
		  if( mllp > 4. && mllp < 100. && fabs( mllp - Zmass ) < fabs( massZ - Zmass ) && RECOPFPHOT_PT->at(iLp[p]) == max_pt_FSR_Z )
{
		...
		if( iLp_l[ p ] == iL[i] && iLp_tagEM[ p ] == 0 ) {
		  ...
		  double mllp = sqrt( pow(E,2) - pow(px,2) - pow(py,2) - pow(pz,2) );
		  double deltaR = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[p]) , RECOMU_PHI->at(iL[i]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[p]) - RECOMU_ETA->at(iL[i]),2) );
		...
		if( iLp_l[ p ] == iL[j] && iLp_tagEM[ p ] == 0 ) {
		  ...
		  double mllp = sqrt( pow(E,2) - pow(px,2) - pow(py,2) - pow(pz,2) );
		  double deltaR = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[p]) , RECOMU_PHI->at(iL[j]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[p]) - RECOMU_ETA->at(iL[j]),2) );
		  if( mllp > 4. && mllp < 100. && fabs( mllp - Zmass ) < fabs( massZ - Zmass ) && deltaR < min_deltaR_FSR_Z){
	  ...
	  if( debug ) cout << "Z Isolation (not corrected for photon): "
			   ...
			   << endl;
	  if( pi != -1 ){
	    //double deltaR_i = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[pi]) , RECOMU_PHI->at(iL[i]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[pi]) - RECOMU_ETA->at(iL[i]),2) );
	    //if( deltaR_i < 0.4 && deltaR_i > 0.01 )
	    pTphot=RECOPFPHOT_PT->at(iLp[pi]);
	  }
	  else if( pj != -1 ){
	    //double deltaR_j = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[pj]) , RECOMU_PHI->at(iL[j]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[pj]) - RECOMU_ETA->at(iL[j]),2) );
	  ...
	  //if( RECOMU_PFX_dB->at( iL[i] ) >= 0.4 || RECOMU_PFX_dB->at( iL[j] ) >= 0.4 ) continue; // cut on isolation
	}
	...
	Z->charge2=RECOMU_CHARGE->at(iL[j]);
      Z->isol1=RECOMU_PFX_dB->at( iL[i] );
      Z->isol2=RECOMU_PFX_dB->at( iL[j] );
	...
	if (Zcandvector.at(index1).withFSR==1 && Zcandvector.at(index2).withFSR==0 ) { // correct the muon isol. of all the 4 leptons for the photon pT asosciated to first Z
	  ...
	  if(Zcandvector.at(index1).ilept1_FSR==true){
	    Zcandvector.at(index1).isol1=(RECOMU_PFchHad->at(Zcandvector.at(index1).ilept1)+
					  max(0.,RECOMU_PFneuHad->at(Zcandvector.at(index1).ilept1)+
					      (RECOMU_PFphoton->at(Zcandvector.at(index1).ilept1)-Zcandvector.at(index1).ptFSR)-
					      0.5*RECOMU_PFPUchAllPart->at(Zcandvector.at(index1).ilept1)))/RECOMU_PT->at(Zcandvector.at(index1).ilept1);
	  }

	  if(Zcandvector.at(index1).ilept2_FSR==true){
	    Zcandvector.at(index1).isol2=(RECOMU_PFchHad->at(Zcandvector.at(index1).ilept2)+
					  max(0.,RECOMU_PFneuHad->at(Zcandvector.at(index1).ilept2)+
					      (RECOMU_PFphoton->at(Zcandvector.at(index1).ilept2)-Zcandvector.at(index1).ptFSR)-
					      0.5*RECOMU_PFPUchAllPart->at(Zcandvector.at(index1).ilept2)))/RECOMU_PT->at(Zcandvector.at(index1).ilept2);
	  }

	  // Zcandvector.at(index2).isol1=(RECOMU_PFchHad->at(Zcandvector.at(index2).ilept1)+
	  // max(0.,RECOMU_PFneuHad->at(Zcandvector.at(index2).ilept1)+
	  // (RECOMU_PFphoton->at(Zcandvector.at(index2).ilept1)-Zcandvector.at(index1).ptFSR)-
	  // 0.5*RECOMU_PFPUchAllPart->at(Zcandvector.at(index2).ilept1)))/RECOMU_PT->at(Zcandvector.at(index2).ilept1);

	  // Zcandvector.at(index2).isol2=(RECOMU_PFchHad[Zcandvector.at(index2).ilept2]+
	  // max(0.,RECOMU_PFneuHad->at(Zcandvector.at(index2).ilept2)+
	  // (RECOMU_PFphoton->at(Zcandvector.at(index2).ilept2)-Zcandvector.at(index1).ptFSR)-
	  // 0.5*RECOMU_PFPUchAllPart->at(Zcandvector.at(index2).ilept2)))/RECOMU_PT->at(Zcandvector.at(index2).ilept2);
	  ...
	  // Zcandvector.at(index1).isol1=(RECOMU_PFchHad->at(Zcandvector.at(index1).ilept1)+
	  // max(0.,RECOMU_PFneuHad->at(Zcandvector.at(index1).ilept1)+
	  // (RECOMU_PFphoton->at(Zcandvector.at(index1).ilept1)-Zcandvector.at(index2).ptFSR)-
	  // 0.5*RECOMU_PFPUchAllPart->at(Zcandvector.at(index1).ilept1)))/RECOMU_PT->at(Zcandvector.at(index1).ilept1);

	  // Zcandvector.at(index1).isol2=(RECOMU_PFchHad->at(Zcandvector.at(index1).ilept2)+
	  // max(0.,RECOMU_PFneuHad->at(Zcandvector.at(index1).ilept2)+
	  // (RECOMU_PFphoton->at(Zcandvector.at(index1).ilept2)-Zcandvector.at(index2).ptFSR)-
	  // 0.5*RECOMU_PFPUchAllPart->at(Zcandvector.at(index1).ilept2)))/RECOMU_PT->at(Zcandvector.at(index1).ilept2);

	  if(Zcandvector.at(index2).ilept1_FSR==true){
	    Zcandvector.at(index2).isol1=(RECOMU_PFchHad->at(Zcandvector.at(index2).ilept1)+
					  max(0.,RECOMU_PFneuHad->at(Zcandvector.at(index2).ilept1)+
					      (RECOMU_PFphoton->at(Zcandvector.at(index2).ilept1)-Zcandvector.at(index2).ptFSR)-
					      0.5*RECOMU_PFPUchAllPart->at(Zcandvector.at(index2).ilept1)))/RECOMU_PT->at(Zcandvector.at(index2).ilept1);
	  }
	  if(Zcandvector.at(index2).ilept2_FSR==true){
	    Zcandvector.at(index2).isol2=(RECOMU_PFchHad->at(Zcandvector.at(index2).ilept2)+
					  max(0.,RECOMU_PFneuHad->at(Zcandvector.at(index2).ilept2)+
					      (RECOMU_PFphoton->at(Zcandvector.at(index2).ilept2)-Zcandvector.at(index2).ptFSR)-
					      0.5*RECOMU_PFPUchAllPart->at(Zcandvector.at(index2).ilept2)))/RECOMU_PT->at(Zcandvector.at(index2).ilept2);
	  }
 ...
 // **** Step 4:
  ...
  for (int vl=0;vl<ileptonsv.size();vl++){
	...
	//commented by sherif
	//if( sqrt( pow( DELTAPHI( RECOMU_PHI->at(i),RECOMU_PHI->at(j) ),2)
	//	  + pow(RECOMU_ETA->at(i) - RECOMU_ETA->at(j),2) ) <= 0.02) continue;
 ...
 // **** Step 6:
  ...
  for(int i = 0; i < N_good; ++i){
    if (RECOMU_SIP->at(iL[i])>=4.) continue;

    for(int j = i + 1; j < N_good; ++j){
      if (RECOMU_SIP->at(iL[j])>=4.) continue;

      if ( RECOMU_CHARGE->at(iL[i]) == RECOMU_CHARGE->at(iL[j])) continue; // shoud be OS
      ...
      E = RECOMU_E->at( iL[i] ) + RECOMU_E->at( iL[j] );

      px = RECOMU_PT->at( iL[i] )*cos( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*cos( RECOMU_PHI->at( iL[j] ) );
      py = RECOMU_PT->at( iL[i] )*sin( RECOMU_PHI->at( iL[i] ) ) + RECOMU_PT->at( iL[j] )*sin( RECOMU_PHI->at( iL[j] ) );
      pz = RECOMU_P->at( iL[i] )*cos( RECOMU_THETA->at( iL[i] ) ) + RECOMU_P->at( iL[j] )*cos( RECOMU_THETA->at( iL[j] ) );
...
if (vindexZZ.size()==1){
  ...
  */
  ...
  for(int i=0;i<RECO_PFJET_N;i++){
	...
	if (RECOELE_PFX_rho->at(iLe[ele])>=0.5) continue; //GIORGIA: sherif comments
  ...
  for(int j=0;j<RECO_PFJET_N;j++){
	...
	if(RECO_PFJET_PT->at(j)>40. && fabs(RECO_PFJET_ETA->at(j))<2.4){
	  ...
	  if (mJJ_VH.M()>60. && mJJ_VH.M()<120.) {
	    cout << "Jets for VH categories passing, pT>40, |eta|<2.4 and 60<mjj<120 with pT= " << RECO_PFJET_PT->at(j) << " " << RECO_PFJET_PT->at(j) << endl;
	    n_jets_mjj_VH++ ;
