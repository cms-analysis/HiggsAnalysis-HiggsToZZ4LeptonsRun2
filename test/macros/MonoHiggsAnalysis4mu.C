#define MonoHiggsAnalysis4mu_cxx
...
void MonoHiggsAnalysis4mu::Loop(Char_t *output)
{
   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     if( datasetName.Contains("amcatnlo")) {      
      ...
      //MC_PDGID->at(0)  the Higgs25
      //MC_PDGID->at(1)  the Z1   23
      //MC_PDGID->at(2)  the Z2   23
      //MC_PDGID->at(3)  the l1+  mu- = 13  e- = 11
      //MC_PDGID->at(4)  the l2-  mu+ = -13  e+ = -11
      //MC_PDGID->at(5)  the l3+  mu- = 13  e- = 11
      //MC_PDGID->at(6)  the l4-  mu+ = -13  e+ = -11
	...
	cout << "Starting weight + pileup + LineShape= " << newweight << endl;
      ...
      if (isSignal){
      	if( MC_PDGID->at(3) == 13 && MC_PDGID->at(4) == -13  
      	    && MC_PDGID->at(5) == 13 && MC_PDGID->at(6) == -13 ){
      	  is4mu=true;
      	  ++N_01 ;  // fill counter 
	...
	for(int i = 0; i < RECO_NMU; ++i){
	  if( RECOMU_PT->at(i) > 17 && !pt_17_emu ){ pt_17_emu = 1; i_17_emu = i; }
	}
	if( !pt_17_emu ) continue;
	for(int i = 0; i < RECO_NMU; ++i){
	  if( i == i_17_emu ) continue;
	  if( RECOMU_PT->at(i) > 8 && !pt_8_emu ){ pt_8_emu = 1; }
	}
	...
	if( ( RECOMU_isGlobalMu->at(i) || (RECOMU_isTrackerMu->at(i) && RECOMU_numberOfMatches->at(i)>0) )
      ...
      cout<<"\n The arraysize_e[0] is: "<< arraysize_e[0] << endl;
    ...
    //////DEBUG FOR RECOMU_PFX_dB_new AND RECOELE_PFX_rho_new
    // cout<<"CHECK OF ENTRY: "<<jentry<<" RUN: "<<Run<< " EVENT: "<<Event<<endl;
    // cout<<"The size of RECOMU_PFX_dB is "<< RECOMU_PFX_dB->size()<<endl;
    // cout<<"The size of RECOMU_PFX_dB_new is "<< RECOMU_PFX_dB_new.size()<<endl;

    // for(int i=0; i<RECOMU_PFX_dB->size(); i++){
    //   cout<<"The element of index "<< i <<" of RECOMU_PFX_dB vector is " << RECOMU_PFX_dB->at(i)<<endl;
    // }
    // for(int i=0; i<RECOMU_PFX_dB_new.size(); i++){
    //   cout<<"The element of index "<< i <<" of RECOMU_PFX_dB_new vector is " << RECOMU_PFX_dB_new.at(i)<<endl;
    // }

    // cout<<"The size of RECOELE_PFX_rho is "<< RECOELE_PFX_rho->size()<<endl;
    // cout<<"The size of RECOELE_PFX_rho_new is "<< RECOELE_PFX_rho_new.size()<<endl;

    // for(int i=0; i<RECOELE_PFX_rho->size(); i++){
    //   cout<<"The element of index "<< i <<" of RECOELE_PFX_rho vector is " << RECOELE_PFX_rho->at(i)<<endl;
    // }
    // for(int i=0; i<RECOELE_PFX_rho_new.size(); i++){
    //   cout<<"The element of index "<< i <<" of RECOELE_PFX_rho_new vector is " << RECOELE_PFX_rho_new.at(i)<<endl;
    // }
    ...
    for( int i = 0; i < RECO_NELE; ++i ){
      ...
      if( debug ) cout << "\n Electron i="<< i <<" properties: "
		       << "\nRECOELE_PT->at(i) " << RECOELE_PT->at(i)
		       << "\nfabs(RECOELE_ETA->at(i)) " << fabs(RECOELE_ETA->at(i))
		       << "\nfabs(RECOELE_scl_Eta->at(i)) " << fabs(RECOELE_scl_Eta->at(i))
	  ...
	  //&& RECOELE_gsftrack_expected_inner_hits->at(i)<=1 NO MORE USED IN SPRING15
	  ) ;
      else continue ;
	...
	if( fabs(RECOELE_scl_Eta->at(i)) >= 1.479 && RECOELE_mvaNonTrigV0->at(i) > -0.267 ) BDT_ok = 1 ;
	  ...
	  && fabs(RECOELE_gsftrack_dz->at(i)) < 1. ) /* ok */;
      else continue ;
      //
      iLe[ Ne_good ] = i ;
      ++Ne_good ;
    ...
    for( int i = 0; i < RECO_NPFPHOT; ++i ){
      ...
      if( debug ) cout << "\n Photon i="<< i <<" properties: "
		       ...
		       << "\n RECOPFPHOT_PFchHad->at(i) " << RECOPFPHOT_PFchHad->at(i) 
		       << "\n RECOPFPHOT_PFneuHad->at(i) " << RECOPFPHOT_PFneuHad->at(i) 
		       << "\n RECOPFPHOT_PFphoton->at(i) " << RECOPFPHOT_PFphoton->at(i) 
		       << "\n RECOPFPHOT_PFPUchAllPart->at(i) "<< RECOPFPHOT_PFPUchAllPart->at(i) 
	...
	if( deltaR<min_deltaR) { // the closest lepton
	  cout << "Possible candidate of photon with pT= " << RECOPFPHOT_PT->at(iLp[i]) << " associated to a muon with pT= " << RECOMU_PT->at(iL_loose_mu[l])<< endl;
	  min_deltaR = deltaR;
	  l_min_deltaR = l;
	  tag_min_deltaR = 0;
	}
	...
	if(!(deltaR < 0.5 && deltaR/pow(RECOPFPHOT_PT->at(iLp[i]),2)<0.012) ) continue;
	if( deltaR<min_deltaR) { // the closest lepton
	  cout << "Possible candidate of photon with pT= " << RECOPFPHOT_PT->at(iLp[i]) << " associated to an electron with pT= " << RECOELE_PT->at(iL_loose_e[l])<< endl;
	  min_deltaR = deltaR;
	  l_min_deltaR = l;
	  tag_min_deltaR = 1;
	}
    ...
    // for(int i=0.;i<Nphotons;i++) {
    //   if (iLp_l[i]!=-1 && iLp_tagEM[i]==0) cout << "There is photon with pT= " << RECOPFPHOT_PT->at(iLp[i]) << " attached to a muon with pT= " << RECOMU_PT->at(iLp_l[i]) << endl;
    // };


    //giorgia: to be continued
	...
	//double deltaR = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[i]) , RECOELE_scl_Phi->at(iL_loose_e[e]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[i]) - RECOELE_scl_Eta->at(iL_loose_e[e]),2) );
	double deltaR = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[i]) , RECOELE_PHI->at(iL_loose_e[e]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[i]) - RECOELE_ETA->at(iL_loose_e[e]),2) );          cout << "deltaR for photon subtraction= " << deltaR << endl;
	
	if( deltaR<=0.3 && (RECOELE_scl_Eta->at(iL_loose_e[e])< 1.479 || deltaR>0.08) ){ // 0.3 is the isolation cone for electrons in 76x 
	...
	if (fabs(RECOMU_SIP->at(iL[i]))>=4.) continue;  // SIP cut for Z1 leptons
	if (fabs(RECOMU_SIP->at(iL[j]))>=4.) continue;
	if (fabs(RECOMU_PFX_dB_new.at(iL[i]))>=0.35) continue; // Isolation cut
	if (fabs(RECOMU_PFX_dB_new.at(iL[j]))>=0.35) continue;

	if(RECOMU_CHARGE->at( iL[j] ) == RECOMU_CHARGE->at( iL[i] )) continue; // opposite charge

	cout << "Pairing muons with pT= " << RECOMU_PT->at( iL[i] ) << " and " <<  RECOMU_PT->at( iL[j] ) << endl;
	...
	Lepton1.SetPtEtaPhiM(RECOMU_PT->at(iL[i]), RECOMU_ETA->at(iL[i]), RECOMU_PHI->at(iL[i]), 0.105);
	Lepton2.SetPtEtaPhiM(RECOMU_PT->at(iL[j]), RECOMU_ETA->at(iL[j]), RECOMU_PHI->at(iL[j]), 0.105);
	...
	if( has_FSR_Z ){ // if Z has FSR
	  ...
	  if( debug ) cout  << "Z Isolation (not corrected for photon): "
			    << "\n RECOMU_PFX_dB->at( iL[i] ) " << RECOMU_PFX_dB->at( iL[i] )
			    << "\n RECOMU_PFX_dB->at( iL[j] ) " << RECOMU_PFX_dB->at( iL[j] )
			    << endl;
	  ...
	  if( debug ) cout  << "Z Isolation: "  
			    << "\n RECOMU_PFX_dB[ iL[i] ] " << RECOMU_PFX_dB->at( iL[i] )
			    << "\n RECOMU_PFX_dB[ iL[j] ] " << RECOMU_PFX_dB->at( iL[j] )
			    << endl;
	...
	Z->ilept1=iL[i];
	Z->ilept2=iL[j];
	Z->pt1=RECOMU_PT->at(iL[i]);
	Z->pt2=RECOMU_PT->at(iL[j]);
	Z->eta1=RECOMU_ETA->at(iL[i]);
	Z->eta2=RECOMU_ETA->at(iL[j]);
	Z->phi1=RECOMU_PHI->at(iL[i]);
	Z->phi2=RECOMU_PHI->at(iL[j]);
	Z->charge1=RECOMU_CHARGE->at(iL[i]);
	Z->charge2=RECOMU_CHARGE->at(iL[j]);
	Z->isol1=RECOMU_PFX_dB_new.at( iL[i] );
	Z->isol2=RECOMU_PFX_dB_new.at( iL[j] );
...
cout << "Checking mass j= " << Zcandisolmassvector.at(j).massvalue << endl;
 ...
 // **** Step 6:
  ...
  for (int l=0;l<ileptonspTcleanedgoodZsv.size();l++){
	...
	if ( RECOMU_CHARGE->at(ileptonsmumu.at(i)) == RECOMU_CHARGE->at(ileptonsmumu.at(j)) ) continue; // shoud be OS	  
	// evaluate the mass
	double mass;
	
	Lepton1qcd.SetPtEtaPhiM(RECOMU_PT->at(ileptonsmumu.at(i)), RECOMU_ETA->at(ileptonsmumu.at(i)), RECOMU_PHI->at(ileptonsmumu.at(i)), 0.105);
	Lepton2qcd.SetPtEtaPhiM(RECOMU_PT->at(ileptonsmumu.at(j)), RECOMU_ETA->at(ileptonsmumu.at(j)), RECOMU_PHI->at(ileptonsmumu.at(j)), 0.105);
  ...
  for (int i=0;i<pTcleanedgoodZ.size();i++){
    ...
    Lepton1.SetPtEtaPhiM(pTcleanedgoodZ.at(i).pt1, pTcleanedgoodZ.at(i).eta1, pTcleanedgoodZ.at(i).phi1, 0.105);
    
    if (pTcleanedgoodZ.at(i).ilept1_FSR) {
      for( int p = 0; p < Nphotons; ++p ){
	if( iLp_l[ p ] == pTcleanedgoodZ.at(i).ilept1 && iLp_tagEM[ p ] == 0 ){
    ...
    Lepton2.SetPtEtaPhiM(pTcleanedgoodZ.at(i).pt2, pTcleanedgoodZ.at(i).eta2, pTcleanedgoodZ.at(i).phi2, 0.105);
    if (pTcleanedgoodZ.at(i).ilept2_FSR) {
      for( int p = 0; p < Nphotons; ++p ){
	if( iLp_l[ p ] == pTcleanedgoodZ.at(i).ilept2 && iLp_tagEM[ p ] == 0 ){
    ...
    for (int j=i+1;j<pTcleanedgoodZ.size();j++){
      ...
      Lepton3.SetPtEtaPhiM(pTcleanedgoodZ.at(j).pt1, pTcleanedgoodZ.at(j).eta1, pTcleanedgoodZ.at(j).phi1, 0.105);
      if (pTcleanedgoodZ.at(j).ilept1_FSR) {
	for( int p = 0; p < Nphotons; ++p ){
	  if( iLp_l[ p ] == pTcleanedgoodZ.at(j).ilept1 && iLp_tagEM[ p ] == 0 ){
      ...
      Lepton4.SetPtEtaPhiM(pTcleanedgoodZ.at(j).pt2, pTcleanedgoodZ.at(j).eta2, pTcleanedgoodZ.at(j).phi2, 0.105);
      if (pTcleanedgoodZ.at(j).ilept2_FSR) {
	for( int p = 0; p < Nphotons; ++p ){
	  if( iLp_l[ p ] == pTcleanedgoodZ.at(j).ilept2 && iLp_tagEM[ p ] == 0 ){
  ...
  if (vindexZZ.size()==1){
	...
	cout << "Passed m4l>70. cut"<< endl;cout << "more than one Z2 (couple to the same Z1), choose the one with the highest-pT Z2 leptons" << endl;
  ...
  cout << "PTs= " << RECOMU_PT->at(indexlep1Z1) << " " << RECOMU_PT->at(indexlep2Z1) << " " << RECOMU_PT->at(indexlep1Z2) << " " << RECOMU_PT->at(indexlep2Z2)<< endl;
  if (std::isnan(massZ2)) {
  ...
  cout << "PTs= " << RECOMU_PT->at(indexleptonfinal[0]) << " " << RECOMU_PT->at(indexleptonfinal[1]) << " " << RECOMU_PT->at(indexleptonfinal[2]) << " " << RECOMU_PT->at(indexleptonfinal[3])<< endl;

  for(int i = 0; i < 4; ++i) tmp_pt[i] = RECOMU_PT->at(indexleptonfinal[i]);
  for(int i = 0; i < 4; ++i){
    double tmp_max_pt = 0;
    int jj = i;
    for(int j = 0; j < 4; ++j){
  ...
  if(debug)
    for(int i = 0; i < 4; ++i)
      cout << "\n ipt[" << i << "] = " << ipt[i] << "\t pt = " << RECOMU_PT->at( ipt[i] ) << endl;
  //end sorting
...
// {run}:{lumi}:{event}:{pdgId}:{pT:.2f}:{eta:.2f}:{phi:.2f}{SIP:.2f}:{PFChargedHadIso:.2f}:{PFNeutralHadIso:.2f}:{PFPhotonIso:.2f}:{PUCorr:.2f}:{combRelIsoPF:.3f}:{eleBDT:.3f}:{photpT:.2f}:{photDR:.2f}:{photRelIso:.2f}
 ...
 Char_t leptformat[20000];
  ...
  for(int i = 0; i < 4; ++i){
	...
	cout << "muons with pT= " << RECOMU_PT->at(ipt[i]) << " has associated a photon with pT= " << RECOPFPHOT_PT->at(iLp[p]) << endl;
	flagFSR=1;
	pfsr=p;
      }
    }
    if (flagFSR==1){
      sprintf (leptformat,"FormatLept=%d:%d:%d:%d:%.2f:%.2f:%.2f:%.2f:%.2f:%.2f:%.2f:%.2f:%.3f:%.3f:%.2f:%.2f:%.2f",
	       ...
	       int(-13*RECOMU_CHARGE->at(ipt[i])),
	       RECOMU_PT->at(ipt[i]),RECOMU_ETA->at(ipt[i]),RECOMU_PHI->at(ipt[i]),RECOMU_SIP->at(ipt[i]),
	       RECOMU_PFchHad->at(ipt[i]),RECOMU_PFneuHad->at(ipt[i]),RECOMU_PFphoton->at(ipt[i]),RECOMU_PFPUchAllPart->at(ipt[i]),RECOMU_PFX_dB->at(ipt[i]),dummy,
    ...
    if( i == 0){
      hPtLep1_5->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep1_5->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep1_5->Fill( RECOMU_PFX_dB->at( ipt[i] ),newweight ) ;
      hSipLep1_5->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep1_5->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep1_5->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 1){
      hPtLep2_5->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep2_5->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep2_5->Fill( RECOMU_PFX_dB->at( ipt[i] ),newweight ) ;
      hSipLep2_5->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep2_5->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep2_5->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 2){
      hPtLep3_5->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep3_5->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep3_5->Fill( RECOMU_PFX_dB->at( ipt[i] ),newweight ) ;
      hSipLep3_5->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep3_5->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep3_5->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 3){
      hPtLep4_5->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep4_5->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep4_5->Fill( RECOMU_PFX_dB->at( ipt[i] ),newweight ) ;
      hSipLep4_5->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep4_5->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep4_5->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
  }//end fill leptons
  ...
  for(int i = 0; i < N_good; ++i){
	...
	cout << "Muon with pT= " << RECOMU_PT->at(iL[i]) << " has associated a photon with pT= " << RECOPFPHOT_PT->at(iLp[p]) << endl;
	// RECOMU_PFX_dB_new[(iL[i])]=
	//   (RECOMU_PFchHad->at(iL[i])+
	//    max(0.,RECOMU_PFneuHad->at(iL[i])+
	//        (RECOMU_PFphoton->at(iL[i])-RECOPFPHOT_PT->at(iLp[p]) )-
	//        0.5*RECOMU_PFPUchAllPart->at(iL[i])))/RECOMU_PT->at(iL[i]);
	flagFSR=1;
	pfsr=p;
      }
    }
    if (flagFSR==1){
      cout << "Before correcting for FSR; muon pT= " << RECOMU_PT->at(iL[i]) << " Eta= " << RECOMU_ETA->at(iL[i]) << " Phi= " << RECOMU_PHI->at(iL[i]) << endl;
      TLorentzVector Lept,LeptCorrection;
      Lept.SetPtEtaPhiM(RECOMU_PT->at(iL[i]), RECOMU_ETA->at(iL[i]), RECOMU_PHI->at(iL[i]), 0.105);
      LeptCorrection.SetPtEtaPhiM(RECOPFPHOT_PT->at(iLp[pfsr]),RECOPFPHOT_ETA->at(iLp[pfsr]),RECOPFPHOT_PHI->at(iLp[pfsr]),0);
      Lept+=LeptCorrection;
      RECOMU_PT->at(iL[i])=Lept.Pt();
      RECOMU_ETA->at(iL[i])=Lept.Eta();
      RECOMU_PHI->at(iL[i])=Lept.Phi();
      cout << "After correcting for FSR; muon pT= " << RECOMU_PT->at(iL[i]) << " Eta= " << RECOMU_ETA->at(iL[i]) << " Phi= " << RECOMU_PHI->at(iL[i]) << endl;
    }
  }
  ...
  */
  ...
  for(int i = 0; i < 4; ++i){
    if( i == 0){
      hPtLep1_7->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep1_7->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep1_7->Fill( RECOMU_PFX_dB_new.at(ipt[i]),newweight ) ;
      hSipLep1_7->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep1_7->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep1_7->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 1){
      hPtLep2_7->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep2_7->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep2_7->Fill( RECOMU_PFX_dB_new.at(ipt[i]),newweight ) ;
      hSipLep2_7->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep2_7->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep2_7->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 2){
      hPtLep3_7->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep3_7->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep3_7->Fill( RECOMU_PFX_dB_new.at(ipt[i]),newweight ) ;
      hSipLep3_7->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep3_7->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep3_7->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 3){
      hPtLep4_7->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep4_7->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep4_7->Fill( RECOMU_PFX_dB_new.at(ipt[i]),newweight ) ;
      hSipLep4_7->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep4_7->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep4_7->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
  }//end fill leptons
  ...
  for(int i = 0; i < 4; ++i){
    if( i == 0){
      hPtLep1_8->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep1_8->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep1_8->Fill( RECOMU_PFX_dB_new.at(ipt[i]),newweight ) ;
      hSipLep1_8->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep1_8->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep1_8->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 1){
      hPtLep2_8->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep2_8->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep2_8->Fill( RECOMU_PFX_dB_new.at(ipt[i]),newweight ) ;
      hSipLep2_8->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep2_8->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep2_8->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 2){
      hPtLep3_8->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep3_8->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep3_8->Fill( RECOMU_PFX_dB_new.at(ipt[i]),newweight ) ;
      hSipLep3_8->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep3_8->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep3_8->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
    if( i == 3){
      hPtLep4_8->Fill( RECOMU_PT->at( ipt[i] ),newweight ) ;
      hYLep4_8->Fill( RECOMU_ETA->at( ipt[i] ),newweight ) ;
      hIsoLep4_8->Fill( RECOMU_PFX_dB_new.at(ipt[i]),newweight ) ;
      hSipLep4_8->Fill( RECOMU_SIP->at( ipt[i] ),newweight ) ;
      hIpLep4_8->Fill( RECOMU_IP->at( ipt[i] ),newweight ) ;
      hIpErLep4_8->Fill( RECOMU_IPERROR->at( ipt[i] ),newweight ) ;
    }
  }//end fill leptons
  ...
  for(int i=0;i<RECO_PFJET_N;i++){
    ...
    //if(RECO_PFJET_PUID->at(i)==1 && RECO_PFJET_PT->at(i)>30. && fabs(RECO_PFJET_ETA->at(i))<4.7 ){ // NO PU ID temporary (PUJETID buggy)
	...
	if (RECOELE_PFX_rho_new.at(iLe[ele])>=0.35) continue; //GIORGIA: sherif comments
 ...
 // Categorization
  ...
  f_lept1_pt = RECOMU_PT->at(indexleptonfinal[0]) ;
  f_lept1_eta = RECOMU_ETA->at(indexleptonfinal[0]) ;
  f_lept1_phi = RECOMU_PHI->at(indexleptonfinal[0]);
  f_lept1_charge = RECOMU_CHARGE->at(indexleptonfinal[0]);
  f_lept1_pfx = RECOMU_PFX_dB_new.at(indexleptonfinal[0]);
  f_lept1_sip = RECOMU_SIP->at(indexleptonfinal[0]);
  // f_lept1_mvaid = RECOMU_mvaNonTrigV0->at(indexleptonfinal[0]);
  f_lept2_pt = RECOMU_PT->at(indexleptonfinal[1]) ;
  f_lept2_eta = RECOMU_ETA->at(indexleptonfinal[1]) ;
  f_lept2_phi = RECOMU_PHI->at(indexleptonfinal[1]);
  f_lept2_charge = RECOMU_CHARGE->at(indexleptonfinal[1]);
  f_lept2_pfx = RECOMU_PFX_dB_new.at(indexleptonfinal[1]);
  f_lept2_sip = RECOMU_SIP->at(indexleptonfinal[1]);
  // f_lept2_mvaid = RECOMU_mvaNonTrigV0->at(indexleptonfinal[1]);
  f_lept3_pt = RECOMU_PT->at(indexleptonfinal[2]) ;
  f_lept3_eta = RECOMU_ETA->at(indexleptonfinal[2]) ;
  f_lept3_phi = RECOMU_PHI->at(indexleptonfinal[2]);
  f_lept3_charge = RECOMU_CHARGE->at(indexleptonfinal[2]);
  f_lept3_pfx = RECOMU_PFX_dB_new.at(indexleptonfinal[2]);
  f_lept3_sip = RECOMU_SIP->at(indexleptonfinal[2]);
  // f_lept3_mvaid = RECOMU_mvaNonTrigV0->at(indexleptonfinal[2]);
  f_lept4_pt = RECOMU_PT->at(indexleptonfinal[3]) ;
  f_lept4_eta = RECOMU_ETA->at(indexleptonfinal[3]) ;
  f_lept4_phi = RECOMU_PHI->at(indexleptonfinal[3]);
  f_lept4_charge = RECOMU_CHARGE->at(indexleptonfinal[3]);
  f_lept4_pfx = RECOMU_PFX_dB_new.at(indexleptonfinal[3]);
  f_lept4_sip = RECOMU_SIP->at(indexleptonfinal[3]);
  // f_lept4_mvaid = RECOMU_mvaNonTrigV0->at(indexleptonfinal[3]);
  // f_iso_max = Iso_max;
  //f_sip_max = Sip_max;
  f_Z1mass = massZ1;
  f_Z2mass = massZ2;
 ...
 //MELA discriminant - Leoton kinematics already corrected for FSR
  ...
  cout << "Final: pT of final leptons from Z1 and Z2 ="
       << RECOMU_PT->at(indexleptonfinal[0]) << " " << RECOMU_PT->at(indexleptonfinal[1]) << " " << RECOMU_PT->at(indexleptonfinal[2]) << " " << RECOMU_PT->at(indexleptonfinal[3]) <<endl;
  L11P4.SetPtEtaPhiM(RECOMU_PT->at(indexleptonfinal[0]), RECOMU_ETA->at(indexleptonfinal[0]), RECOMU_PHI->at(indexleptonfinal[0]), 0.105);
  L12P4.SetPtEtaPhiM(RECOMU_PT->at(indexleptonfinal[1]), RECOMU_ETA->at(indexleptonfinal[1]), RECOMU_PHI->at(indexleptonfinal[1]), 0.105);
  L21P4.SetPtEtaPhiM(RECOMU_PT->at(indexleptonfinal[2]), RECOMU_ETA->at(indexleptonfinal[2]), RECOMU_PHI->at(indexleptonfinal[2]), 0.105);
  L22P4.SetPtEtaPhiM(RECOMU_PT->at(indexleptonfinal[3]), RECOMU_ETA->at(indexleptonfinal[3]), RECOMU_PHI->at(indexleptonfinal[3]), 0.105);
  ...
  // cout << "Muon with pT= " << RECOMU_PT->at(indexleptonfinal[i]) << " has associated a photon with pT= " << RECOPFPHOT_PT->at(iLp[p]) << endl;
  // flagFSR=1;
  // pfsr=p;
  // }
  // }
  ...
  if (debug) for(unsigned int i=0; i< 4; i++) output_txt << "\t mu[" << i << "] pT: " << RECOMU_PT->at(indexleptonfinal[i]) << "\t eta: " << RECOMU_ETA->at(indexleptonfinal[i]) << "\t phi: " << RECOMU_PHI->at(indexleptonfinal[i]) << endl;
...
double MonoHiggsAnalysis4mu::DELTAPHI( double phi1, double phi2 ){
...
double MonoHiggsAnalysis4mu::invmass (float M1, float PT1, float ETA1, float PHI1, float M2, float PT2, float ETA2, float PHI2 ){
  float phi1=PHI1;
  float eta1=ETA1;
  float pt1=PT1;
  float m1=M1;
...
double MonoHiggsAnalysis4mu::masserror( std::vector<TLorentzVector> Lep, std::vector<double> pterr){
...
double MonoHiggsAnalysis4mu::EAele(int index,bool use2011EA){
