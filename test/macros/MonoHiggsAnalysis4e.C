...
//	
 ...
 // counter weighted
  ...
  TH1F * hMZ1BB_3 = new TH1F("hMZ1BB_3", "Mass of Z1 after selection step 3 barrel barrel", 200 , -0.5 , 199.5 );
  hMZ1BB_3->SetXTitle("mass_Z1 barrel barrel  (GeV)");
  TH1F * hMZ1EE_3 = new TH1F("hMZ1EE_3", "Mass of Z1 after selection step 3 endcap endcap", 200 , -0.5 , 199.5 );
  hMZ1EE_3->SetXTitle("mass_Z1 endcap endcap  (GeV)");
  ...
  */
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if( isSignal && debug ) cout << "\n ** Step 0.1 (MC truth): " 
    				 << "\nMC_PDGID->at(0) " << MC_PDGID->at(0)
    				 << "\nMC_PDGID->at(1) " << MC_PDGID->at(1)	
    				 << "\nMC_PDGID->at(2) " << MC_PDGID->at(2)	
    				 << "\nMC_PDGID->at(3) " << MC_PDGID->at(3)	
    				 << "\nMC_PDGID->at(4) " << MC_PDGID->at(4)	
    				 << "\nMC_PDGID->at(5) " << MC_PDGID->at(5)	
    				 << "\nMC_PDGID->at(6) " << MC_PDGID->at(6)	
    				 << endl ;
 ...
 //////////// (VBF) Line Shape Correction///////////
	...
	if( RECOMU_isPFMu->at(mu)
	    ...
	    && fabs(RECOMU_SIP->at(mu))<4. // TightID + SIP cut
	    );
	else continue;
    ...
    for( int i = 0; i < RECO_NELE; ++i ){
      ...
      if( debug ) cout << "\n Electron i="<< i <<" properties: "
		       << "\nRECOELE_PT->at(i) " << RECOELE_PT->at(i)
		       << "\nfabs(RECOELE_ETA->at(i)) " << fabs(RECOELE_ETA->at(i))
		       << "\nfabs(RECOELE_scl_Eta[i]) " << fabs(RECOELE_scl_Eta->at(i))
		       << "\nRECOELE_PFX_rho->at(i) " << RECOELE_PFX_rho->at(i)
		       << "\n chHad= " <<  RECOELE_PFchHad->at(i)                       
		       << "\n neuHad= " << RECOELE_PFneuHad->at(i)
		       << "\n photon= " <<  RECOELE_PFphoton->at(i)
		       << "\n PU= " << RECOMU_PFPUchAllPart->at(i)  
      ...
      if( RECOELE_PT->at(i) > 7. && RECOELE_PT->at(i) <= 10. ){
	if( fabs(RECOELE_scl_Eta->at(i)) < .8 && RECOELE_mvaNonTrigV0->at(i) > -0.265 ) BDT_ok = 1 ;
	if( ( fabs(RECOELE_scl_Eta->at(i)) >= .8 && fabs(RECOELE_scl_Eta->at(i)) < 1.479 )
	    && RECOELE_mvaNonTrigV0->at(i) > -0.556 ) BDT_ok = 1 ;
	if( fabs(RECOELE_scl_Eta->at(i)) >= 1.479 && RECOELE_mvaNonTrigV0->at(i) > -0.551 ) BDT_ok = 1 ;
      }
      else {
	if( fabs(RECOELE_scl_Eta->at(i)) < .8 && RECOELE_mvaNonTrigV0->at(i) > -0.072 ) BDT_ok = 1 ;
	if( ( fabs(RECOELE_scl_Eta->at(i)) >= .8 && fabs(RECOELE_scl_Eta->at(i)) <= 1.479 )
	    && RECOELE_mvaNonTrigV0->at(i) > -0.286 ) BDT_ok = 1 ;
	if( fabs(RECOELE_scl_Eta->at(i)) > 1.479 && RECOELE_mvaNonTrigV0->at(i) > -0.267 ) BDT_ok = 1 ;
      }
      if( !BDT_ok ) continue ;
    ...
    //GIORGIA: TO BE CHECKED
    RECOMU_PFX_dB_new.clear();
    RECOELE_PFX_rho_new.clear();

    RECOMU_PFX_dB_new.reserve( RECOMU_PFX_dB->size() );
    RECOELE_PFX_rho_new.reserve( RECOELE_PFX_rho->size() );

    copy(RECOMU_PFX_dB->begin(), RECOMU_PFX_dB->end(), std::back_inserter(RECOMU_PFX_dB_new));
    copy(RECOELE_PFX_rho->begin(), RECOELE_PFX_rho->end(), std::back_inserter(RECOELE_PFX_rho_new));
    ...
    for( int i = 0; i < RECO_NPFPHOT; ++i ){
      ...
      if ( RECOPFPHOT_PT->at(i) > 2. && fabs(RECOPFPHOT_ETA->at(i)) < 2.4 && RECOPFPHOT_PFX_rho->at(i)<1.8 ) {
	...
	if(!(deltaR < 0.5 && deltaR/pow(RECOPFPHOT_PT->at(iLp[i]),2)<0.012) ) continue;
	if( deltaR/pow(RECOPFPHOT_PT->at(iLp[i]),2)<min_deltaR_ET2) {
	    min_deltaR = deltaR;
	    l_min_deltaR = l;
	    tag_min_deltaR = 0;
	  }
	...
	if (fabs(RECOELE_SIP->at(iL_loose_e[l]))>=4.) continue;  //loose ID + SIP cut	  
    ...
    // for(int i=0.;i<Nphotons;i++) {
    //   if (iLp_l[i]!=-1 && iLp_tagEM[i]==0) cout << "There is photon with pT= " << RECOPFPHOT_PT->at(iLp[i]) << " attached to an electron with pT= " << RECOELE_PT->at(iLp_l[i]) << endl;
    // };
    ...
    for(int l = 0; l < N_loose_mu; ++l){ // loop on muons
      if (fabs(RECOMU_SIP->at(iL_loose_mu[l]))>=4.) continue; //loose ID + SIP cut
      min_deltaR_ET2=1000;
      p_min_deltaR_ET2=-1;
	...
	if( iLp_l[ p ] == iL_loose_mu[l] && iLp_tagEM[ p ] == 0 )  {
	  ...
	  double deltaR = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[p]) , RECOMU_PHI->at(iL_loose_mu[l]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[p]) - RECOMU_ETA->at(iL_loose_mu[l]),2) );
	  double deltaR_ET2 = deltaR/pow(RECOPFPHOT_PT->at(iLp[p]),2);
	...
	if( iLp_l[ p ] == iL_loose_e[l] && iLp_tagEM[ p ] == 1 )  {
	  ...
	  double deltaR = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[p]) , RECOELE_PHI->at(iL_loose_e[l]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[p]) - RECOELE_ETA->at(iL_loose_e[l]),2));
	  double deltaR_ET2 = deltaR/pow(RECOPFPHOT_PT->at(iLp[p]),2);
    ...
    for(int i=0.;i<Nphotons;i++) {
      if (iLp_l[i]!=-1 && iLp_tagEM[i]==1) cout << "There is photon with pT= " << RECOPFPHOT_PT->at(iLp[i]) << " attached to an electron with pT= " << RECOELE_PT->at(iLp_l[i]) << endl;
    };           
	...
	for(int e = 0; e < N_loose_e; ++e){
	  ...
	  double deltaR = sqrt( pow( DELTAPHI( RECOPFPHOT_PHI->at(iLp[i]) , RECOELE_PHI->at(iL_loose_e[e]) ),2) + pow(RECOPFPHOT_ETA->at(iLp[i]) - RECOELE_ETA->at(iL_loose_e[e]),2) );
	  cout << "deltaR for photon subtraction= " << deltaR << endl;
	  if( deltaR<=0.3 && (RECOELE_scl_Eta->at(iL_loose_e[e])< 1.479 || deltaR>0.08) ){ // 0.4 is the isolation cone for electrons in 74x -> 0.3 in 76x              
	    ...
	    RECOELE_PFX_rho_new.at(iL_loose_e[e])=
              (RECOELE_PFchHad->at(iL_loose_e[e])+
               max(0.,RECOELE_PFneuHad->at(iL_loose_e[e])+
                   (RECOELE_PFphoton->at(iL_loose_e[e])-RECOPFPHOT_PT->at(iLp[i]) )-
                   max(RHO_ele,0.0)*(EffectiveArea)))/RECOELE_PT->at(iL_loose_e[e]);	    
	  }
	} // end loop on ele
	...
	for(int l = 0; l < N_loose_mu; ++l){ // loop on muons
	  ...
	  if( deltaR<=0.3 && deltaR>0.01){ // 0.3 is the isolation cone for muons in 76x
	    ...
	    RECOMU_PFX_dB_new.at(iL_loose_mu[l])=
              (RECOMU_PFchHad->at(iL_loose_mu[l])+
               max(0.,RECOMU_PFneuHad->at(iL_loose_mu[l])+
                   (RECOMU_PFphoton->at(iL_loose_mu[l])-RECOPFPHOT_PT->at(iLp[i]) )-
                   0.5*RECOMU_PFPUchAllPart->at(iL_loose_mu[l])))/RECOMU_PT->at(iL_loose_mu[l]);
	...
	for(int j = i + 1; j < Ne_good; ++j){
	  ...
	  if (fabs(RECOELE_PFX_rho->at(iLe[i]))>=0.35) continue; // Isolation cut
	  if (fabs(RECOELE_PFX_rho->at(iLe[j]))>=0.35) continue;
	  
	  if(RECOELE_CHARGE->at( iLe[j] ) == RECOELE_CHARGE->at( iLe[i] )) continue; // opposite charge

	  cout << "Pairing electrons with pT= " << RECOELE_PT->at( iLe[i] ) << " and " <<  RECOELE_PT->at( iLe[j] ) << endl;
	  ...
	  Lepton1.SetPtEtaPhiM(RECOELE_PT->at(iLe[i]), RECOELE_ETA->at(iLe[i]), RECOELE_PHI->at(iLe[i]), 0.000511);
	  Lepton2.SetPtEtaPhiM(RECOELE_PT->at(iLe[j]), RECOELE_ETA->at(iLe[j]), RECOELE_PHI->at(iLe[j]), 0.000511);
	...
	for( int p = 0; p < Nphotons; ++p ){
	  if( iLp_l[ p ] == iLe[i] && iLp_tagEM[ p ] == 1 ) { // exit a photon associated to a lepton electron
	    ...
	    // evaluate the mass
	    LeptonCorrection.SetPtEtaPhiM(RECOPFPHOT_PT->at(iLp[p]),RECOPFPHOT_ETA->at(iLp[p]),RECOPFPHOT_PHI->at(iLp[p]),0);
	...
	Z->isol1=RECOELE_PFX_rho_new.at( iLe[i] );
	Z->isol2=RECOELE_PFX_rho_new.at( iLe[j] );
    ...
    */
	...
	for (int j=i+1;j<4;j++){
	  ...
	  if ( RECOELE_CHARGE->at(ileptonsee.at(i)) == RECOELE_CHARGE->at(ileptonsee.at(j)) ) continue; // shoud be OS	  
	  // evaluate the mass
	  double mass;
	  
	  Lepton1qcd.SetPtEtaPhiM(RECOELE_PT->at(ileptonsee.at(i)), RECOELE_ETA->at(ileptonsee.at(i)), RECOELE_PHI->at(ileptonsee.at(i)), 0.000511);
	  Lepton2qcd.SetPtEtaPhiM(RECOELE_PT->at(ileptonsee.at(j)), RECOELE_ETA->at(ileptonsee.at(j)), RECOELE_PHI->at(ileptonsee.at(j)), 0.000511);
	...
	for( int p = 0; p < Nphotons; ++p ){
	  if( iLp_l[ p ] == pTcleanedgoodZ.at(i).ilept1 && iLp_tagEM[ p ] == 1 ){
	    Lepton1Correction.SetPtEtaPhiM(RECOPFPHOT_PT->at(iLp[p]),RECOPFPHOT_ETA->at(iLp[p]),RECOPFPHOT_PHI->at(iLp[p]),0);
	...
	for( int p = 0; p < Nphotons; ++p ){
	  if( iLp_l[ p ] == pTcleanedgoodZ.at(i).ilept2 && iLp_tagEM[ p ] == 1 ){
	    Lepton2Correction.SetPtEtaPhiM(RECOPFPHOT_PT->at(iLp[p]),RECOPFPHOT_ETA->at(iLp[p]),RECOPFPHOT_PHI->at(iLp[p]),0);
	...
	if (pTcleanedgoodZ.at(j).ilept1_FSR) {
	  for( int p = 0; p < Nphotons; ++p ){
	    if( iLp_l[ p ] == pTcleanedgoodZ.at(j).ilept1 && iLp_tagEM[ p ] == 1 ){
	      Lepton3Correction.SetPtEtaPhiM(RECOPFPHOT_PT->at(iLp[p]),RECOPFPHOT_ETA->at(iLp[p]),RECOPFPHOT_PHI->at(iLp[p]),0);
	...
	if (pTcleanedgoodZ.at(j).ilept2_FSR) {
	  for( int p = 0; p < Nphotons; ++p ){
	    if( iLp_l[ p ] == pTcleanedgoodZ.at(j).ilept2 && iLp_tagEM[ p ] == 1 ){
	      Lepton4Correction.SetPtEtaPhiM(RECOPFPHOT_PT->at(iLp[p]),RECOPFPHOT_ETA->at(iLp[p]),RECOPFPHOT_PHI->at(iLp[p]),0);
	...
	Lept.SetPtEtaPhiM(RECOELE_PT->at(iLe[i]), RECOELE_ETA->at(iLe[i]), RECOELE_PHI->at(iLe[i]), 0.000511);
 ...
 // **** Step 8:
	...
	for(int mu = 0; mu < N_good; ++mu){
	  ...
	  if (RECOMU_PFX_dB_new.at(iL[mu])>=0.35) continue;
	...
	for(int ele = 0; ele < Ne_good; ++ele){
	  ...
	  if (RECOELE_PFX_rho_new.at(iLe[ele])>=0.35) continue;
	...
	if (cSV_BTagJet_PT->at(j)==RECO_PFJET_PT->at(i) && cSV_BTagJet_DISCR->at(j)>0.89 && cSV_BTagJet_PT->at(j)>30. && fabs(cSV_BTagJet_ETA->at(j))<4.7) {
	  //if (cSV_BTagJet_DISCR->at(j)>0.89 && cSV_BTagJet_PT->at(j)>30. && fabs(cSV_BTagJet_ETA->at(j))<4.7) {
	  n_match_bjets++;
...
double MonoHiggsAnalysis4e::EAele(int index,bool use2011EA){
  ...
  // }
    ...
    if (fabs(RECOELE_scl_Eta->at(index)) >= 0.0   && fabs(RECOELE_scl_Eta->at(index)) < 1.0 )   EffectiveArea = 0.1752;
    if (fabs(RECOELE_scl_Eta->at(index)) >= 1.0   && fabs(RECOELE_scl_Eta->at(index)) < 1.479 ) EffectiveArea = 0.1862;
    if (fabs(RECOELE_scl_Eta->at(index)) >= 1.479 && fabs(RECOELE_scl_Eta->at(index)) < 2.0 )   EffectiveArea = 0.1411;
    if (fabs(RECOELE_scl_Eta->at(index)) >= 2.0   && fabs(RECOELE_scl_Eta->at(index)) < 2.2 )   EffectiveArea = 0.1534;
    if (fabs(RECOELE_scl_Eta->at(index)) >= 2.2   && fabs(RECOELE_scl_Eta->at(index)) < 2.3 )   EffectiveArea = 0.1903;
    if (fabs(RECOELE_scl_Eta->at(index)) >= 2.3   && fabs(RECOELE_scl_Eta->at(index)) < 2.4 )   EffectiveArea = 0.2243;
    if (fabs(RECOELE_scl_Eta->at(index)) >= 2.4   && fabs(RECOELE_scl_Eta->at(index)) < 5.0  )  EffectiveArea = 0.2687;
  }
