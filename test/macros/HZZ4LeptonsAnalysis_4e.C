...
//   TFile *mu_scale_factors = new TFile("SF2013_V5_old2011.root");
   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      //if (!(Run==1 && Event==325784 && LumiSection==3258)) continue;
	...
	float ptFSR;
	  ...
	  if( has_FSR_Z ){ // if Z has FSR
	    ...
	    //if(  RECOELE_PFX_rho[ iLe[i] ] >= 0.4 ||  RECOELE_PFX_rho[ iLe[j] ] >= 0.4 ) continue;   // cut on isolation
	  }
	...
	if(RECO_PFJET_PUID[i]==1 && RECO_PFJET_PT[i]>30. && fabs(RECO_PFJET_ETA[i])<4.7 ){
	  //Check that jet has deltaR>0.4 away from any tight lepton 
	   ...
	   for(int mu = 0; mu < N_good; ++mu){
            ...
            if (RECOMU_PFX_dB[iL[mu]]>=0.4) continue;
